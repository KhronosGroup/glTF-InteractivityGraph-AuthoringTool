...
### Sample Asset Tests
These tests use the Khronos/Needle glTF Interactivity Sample Assets test catalog. By default the harness looks for the asset repo next to this project at `../glTF-Interactivity-Sample-Assets`; override it with `KHR_INTERACTIVITY_SAMPLE_ASSETS=/path/to/glTF-Interactivity-Sample-Assets`.
...
- `npm run test:assets:e2e` runs the same assets through a lightweight, browserless Babylon engine world.
- `npm run test:assets` runs both asset suites.
