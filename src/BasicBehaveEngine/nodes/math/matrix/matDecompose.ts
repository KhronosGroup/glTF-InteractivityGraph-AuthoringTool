...
export class MatDecompose extends BehaveEngineNode {
    ...
    override processNode(flowSocket?: string) {
        ...
        if (a[3][0] !== 0 || a[3][1] !== 0 || a[3][2] !== 0 || a[3][3] !== 1) {
            result.isValid.value = false;
            return result;
        }

        const s_x = Math.sqrt(a[0][0] * a[0][0] + a[1][0] * a[1][0] + a[2][0] * a[2][0]);
        const s_y = Math.sqrt(a[0][1] * a[0][1] + a[1][1] * a[1][1] + a[2][1] * a[2][1]);
        const s_z = Math.sqrt(a[0][2] * a[0][2] + a[1][2] * a[1][2] + a[2][2] * a[2][2]);
